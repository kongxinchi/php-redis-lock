...
use Predis\Client;
use RedisLock\Client\PredisAdapter;
...
class RedisLockWithPredisTest extends \PHPUnit_Framework_TestCase {
    ...
    public static function setUpBeforeClass() {
        $client = new Client(['tcp://'.REDIS_TEST_SERVER.'?persistent=1'], ['cluster' => 'redis']);
        self::$Redis = new PredisAdapter($client);
    }

    public function testRedis() {
        $this->assertInstanceOf(RedisClientInterface::class, static::$Redis);
    }

    public function setUp() {
        (new Client('tcp://'.REDIS_TEST_SERVER.'?persistent=1'))->flushall();
    }
